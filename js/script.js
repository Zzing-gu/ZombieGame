...
function realtime(){
		...
		for (var i = 0; i < missleList.length; i++) {
			...
			ctx.drawImage(missle,missleList[i].x +30,missleList[i].y+55,10,10);
		}
		createMissle();
		moveMissle();
...
document.querySelectorAll("body")[0]
	...
	var createMissle = function(){
		...
		var originX = player_x;
		var originY = player_y;

		var x = player_x;
		var y = player_y;

		var distantX = mouseX - player_x;
		var distantY = mouseY - player_y;
	...
	var moveMissle = function(){
		for (var i = 0; i < missleList.length; i++) {
			missleList[i].x += missleList[i].distantX/missleList[i].pita  * 20;
			missleList[i].y += missleList[i].distantY/missleList[i].pita   *20;
		}
	};
